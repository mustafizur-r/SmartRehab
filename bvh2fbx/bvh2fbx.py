...
# ---------------------------------------------------------------------
# NEW: Dynamic frame count — reads directly from BVH file
# Falls back to 268 if file is unreadable
# ---------------------------------------------------------------------
def read_bvh_frame_count(bvh_path: str, fallback: int = 268) -> int:
    """Read the 'Frames: N' line from a BVH file and return N."""
    try:
        ...
        log(f"[BVH] Could not read frame count from {bvh_path}: {e} — using fallback {fallback}")
    return fallback


# ---------------------------------------------------------------------
# NEW: Video title — reads video_title.txt if it exists, else input.txt
# video_title.txt is written by app_server.py for refinement steps
# ---------------------------------------------------------------------
def read_video_title() -> str:
    """
    Priority:
      1. video_title.txt  — written by /refine_motion/ with current impairment description
      2. input.txt        — original user prompt (written by /gen_text2motion/)
    """
...
# ---------------------------------------------------------------------
# Step 3: Configure read-mapping settings  (DYNAMIC frame count)
# ---------------------------------------------------------------------
def readBone_setup_scene_settings(num_frames: int):
    ensure_keemap_settings()
    ks = bpy.context.scene.keemap_settings
    ks.start_frame_to_apply         = 1
    ks.number_of_frames_to_apply    = num_frames      # ← was hardcoded 268
    ks.keyframe_every_n_frames      = 1
    ks.bone_mapping_file            = BONE_MAPPING_FILE_PATH
    log(f"[Setup] number_of_frames_to_apply = {num_frames}")
...
def detect_contact_frame_by_scan(armature_name, frame_start=1, frame_end=268, step=1, floor_z=None, prefer_above=True):
    ...
    for f in range(frame_start, frame_end + 1, step):
        ...
        if prefer_above:
            score = z - floor_z if z >= floor_z else (floor_z - z) + 1e6
        else:
            score = abs(z - floor_z)
...
def find_primary_character_armature():
    ...
    best = scored[0][3]
    log(f"[Video] Selected avatar armature: {best.name} "
        f"(driven_meshes={scored[0][0]}, has_action={bool(scored[0][1])})")
    return best
...
# ---------------------------------------------------------------------
# VSE scene with 2D title overlay
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Video export  (DYNAMIC frame range)
# ---------------------------------------------------------------------
def export_video_with_title(
    ...
    resolution_y=1080,
    fps=30,
    frame_end=None,        # ← NEW: pass dynamic frame count; if None, reads from BVH
):
...
# ---------------------------------------------------------------------
# Main workflow  (DYNAMIC frame count throughout)
# ---------------------------------------------------------------------
def main():
    log("Script started")

    # ── Read frame count from BVH before anything else ───────────────
    ...
    readBone_setup_scene_settings(num_frames)   # ← dynamic
    keemap_read_file()
...
if __name__ == "__main__":
    ...
    if args.video_render.lower() == "true":
        # ── Read title: video_title.txt (refinement) → input.txt (base) ──
        title_text = read_video_title()

        # ── Frame count is read from the BVH file automatically ──────────
        export_video_with_title(
            ...
            resolution_y=1080,
            fps=30,
            frame_end=None,   # None = auto-detect from BVH
        )